const generarTextoACopiar = (productos, nombre, apellido, direccion) => {
  ...
  for (const prod of productos) {
    ...
    texto += `📦 - ${prod.titulo} - Cant.: ${prod.cantidad} - Precio Subtotal: $${totalProducto}\n`;
    texto += `  Descripción: ${prod.descrip}\n`;
  }
  texto += `\n💵 Total de servicios: $${totalPagar}`;
  return texto;
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const mostrarTotal = () => {
    ...
    const totalHTML = `
      <div class="total">
        <h4>Total de servicios: $${totalPagar}</h4>
      </div>
    `;
  ...
  const enviarPorWhatsApp = () => {
    ...
    Swal.fire({
      title: "Quiero saber más sobre estos servicios",
      html: mensaje,
      icon: "info",
      showCancelButton: true,
      confirmButtonText: "Volver a Home",
      cancelButtonText: "Copiar y enviar por WhatsApp",
      customClass: {
